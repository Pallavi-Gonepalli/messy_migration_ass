...
1. Fixed JSON Decode Errors
Issue: Using json.loads(request.get_data()) caused crashes when the body was empty or improperly formatted.

Fix: Replaced with request.get_json(silent=True) and added None checks to handle missing or invalid JSON gracefully.

2. Secured Against SQL Injection
Issue: SQL queries were constructed using string interpolation (%s), which is unsafe.

Fix: All queries now use parameterized statements (?) with bound parameters to prevent injection.

3. Password Hashing for Security
Issue: Passwords were stored as plain text.

Fix: Passwords are now hashed using werkzeug.security.generate_password_hash() during registration and checked using check_password_hash() during login.

4. Improved Input Validation
Issue: API did not check for missing or invalid fields (e.g. email format).
...
Issue: Responses were sometimes plain strings or inconsistent.

Fix: All API responses now return consistent JSON format with proper keys (message, error, status, etc.).